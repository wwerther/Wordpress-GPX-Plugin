...
#        var_export ($this->nodes);

        $trackno=0;
        $this->tracks=Array();
        foreach ($this->gpx->trk as $track) {

            $this->tracks[$trackno]=Array( 
                                        'segments' => array(), 
                                        'attribute' => null
                                         );
            $this->tracks[$trackno]['attribute']['name']=(string)$track->name;
            $ext=$track->extensions->children('http://www.garmin.com/xmlschemas/GpxExtensions/v3');
            $this->tracks[$trackno]['attribute']['displaycolor']=(string)$ext->TrackExtension->DisplayColor;

            $trksegno=0;
            foreach ($track->trkseg as $trkseg) {
                $this->tracks[$trackno]['segments'][$trksegno]=array( 'points' => array() );

                $trkptno = 0;
                foreach ($trkseg->trkpt as $trkpt) {
                    $point['lat']=(float)$trkpt['lat'];
                    $point['lon']=(float)$trkpt['lon'];
                    $point['ele']=(float)$trkpt->ele;
                    $point['time']=strtotime($trkpt->time);

                    if (isset($trkpt->extensions)) {
                        $ext=$trkpt->extensions->children('http://www.garmin.com/xmlschemas/TrackPointExtension/v1');
                        $point['hr'] = (float)$ext->TrackPointExtension->hr;
		                $point['cad'] = (float)$ext->TrackPointExtension->cad;
                    }

                    if ($trkptno>0) {
                        $last=$this->tracks[$trackno]['segments'][$trksegno]['points'][$trkptno-1];
                        $point['interval']=abs($point['time']-$last['time']);
                        $point['height']=$point['ele']-$last['ele'];
                        $point['distance']=GPX_helper::distance($last['lat'],$last['lon'],$point['lat'],$point['lon'])*GPX_RADIUS;
                    
                        $point['speed']=null;
                        # $point['totalinterval']=$point['interval']+$last['totalinterval'];
                       if ($point['interval']>0) {
                            $point['speed']=($point['distance']/$point['interval'])*3.6;
                            if ($point['speed']>0) {
                                $point['pace']=(1/ $point['speed']*60);
                            } else {
                                $point['pace']=null;
                            }
                        }
                    ...
                    array_push($this->tracks[$trackno]['segments'][$trksegno]['points'],$point);
    ...
    public function trackcount() {
        return count($this->gpx->trk);
    }

    public function ns() {
        return $this->gpx->getDocNamespaces();
    }
...
$gpx=new test('/var/www/wordpress/wp-content/plugins/gpx2chart/test/hoehenlinie.gpx');
...
$gpx=new test('/var/www/wordpress/wp-content/plugins/gpx2chart/test/bigfile.gpx');
#$gpx=new test('/var/www/wordpress/wp-content/plugins/gpx2chart/test/cadence.gpx');
#var_dump($gpx->ns());
